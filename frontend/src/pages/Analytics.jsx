...
export default function Analytics() {
  ...
  const th = {
    ...
    surface:     isDark ? 'rgba(255,255,255,0.06)' : '#f1f5f9',
    inputBg:     isDark ? 'rgba(255,255,255,0.06)' : '#fff',
    inputBorder: isDark ? 'rgba(255,255,255,0.1)'  : '#ececf3',
    divider:     isDark ? 'rgba(255,255,255,0.07)' : '#f1f5f9',
    rowHover:    isDark ? 'rgba(124,58,237,0.12)'  : '#faf5ff',
    barEmpty:    isDark ? 'rgba(255,255,255,0.08)' : '#eef2f7',
  }
  ...
  // ── Metrics ──
  const parseTs = (ts) => { const d = new Date(ts); return isNaN(d) ? null : d }
  const daysAgo = (ts) => { const d = parseTs(ts); return d ? (Date.now() - d.getTime()) / 86400000 : Infinity }
  ...
  const weekly = Array.from({ length: 7 }, (_, i) => {
    ...
    const count = photos.filter(p => { const ts = parseTs(p.timestamp); return ts && ts >= d && ts < next }).length
  ...
  const KPIS = [
    ...
    { label: 'Active Profiles', val: profiles.length,  icon: '👥', tint: '#10b981', sub: 'profiles tracked',       subColor: th.text3 },
  ...
  return (
    <div style={{ padding: '20px 28px', height: '100vh', overflowY: 'auto', background: th.pageBg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 800, color: th.text1, letterSpacing: '-0.6px' }}>Analytics</div>
          <div style={{ fontSize: 13.5, color: th.text2, marginTop: 2 }}>A detailed look at your GeoTag activity</div>
        </div>
        <div style={{ marginLeft: 'auto', position: 'relative', minWidth: 240 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search uploads…"
            style={{ width: '100%', padding: '10px 12px 10px 34px', borderRadius: 12, border: `1px solid ${th.inputBorder}`, background: th.inputBg, color: th.text1, fontSize: 13.5, outline: 'none' }} />
        </div>
      </div>

      {loading ? (
        <div style={{ padding: 60, textAlign: 'center', color: th.text3 }}>Loading analytics…</div>
      ) : (
        <>
          {/* KPI cards */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 16, marginBottom: 16 }}>
            {KPIS.map((k, i) => (
              <div key={i} style={{ ...th.card, padding: 18 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ fontSize: 12.5, color: th.text2, fontWeight: 600 }}>{k.label}</div>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, color: th.text1, letterSpacing: '-1px', marginTop: 8 }}>{k.val.toLocaleString()}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1fr) 1.6fr', gap: 16, marginBottom: 16 }}>
            ...
            {/* Photos by category */}
            <div style={{ ...th.card, padding: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 800, color: th.text1 }}>Photos by Category</div>
                  <div style={{ fontSize: 12, color: th.text3 }}>Distribution across priorities</div>
                </div>
                <div style={{ marginLeft: 'auto', fontSize: 12, color: th.text2, fontWeight: 700, background: th.surface, padding: '5px 12px', borderRadius: 9 }}>
                  {total} total
                </div>
              </div>
              {(() => {
                        ...
                        <div key={c.value} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%' }}>
                          ...
                          <div style={{ flex: 1, width: '100%', maxWidth: 54, margin: '0 auto', display: 'flex', alignItems: 'flex-end' }}>
                            <div title={`${c.label}: ${c.n} (${pct}%)`} style={{
                              ...
                              background: c.n > 0 ? `linear-gradient(180deg, ${c.color}, ${c.color}cc)` : th.barEmpty,
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                            ...
                            <span style={{ fontSize: 11.5, color: th.text4, fontWeight: 600 }}>{c.label}</span>
                          </div>
                        </div>
                      )
                    })}
          ...
          {/* Bottom row: category breakdown + recent uploads */}
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 1fr) 2fr', gap: 16 }}>
            {/* Category breakdown */}
            <div style={{ ...th.card, padding: 18 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: th.text1, marginBottom: 14 }}>By Category</div>
              {catCounts.map(c => {
                const pct = total ? Math.round((c.n / total) * 100) : 0
                return (
                  <div key={c.value} style={{ marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', fontSize: 13, marginBottom: 5 }}>
                      ...
                      <span style={{ fontWeight: 700, color: th.text1 }}>{c.label}</span>
                      <span style={{ marginLeft: 'auto', color: th.text2, fontWeight: 600 }}>{c.n} · {pct}%</span>
                    </div>
                    <div style={{ height: 8, borderRadius: 999, background: th.surface, overflow: 'hidden' }}>
            ...
            <div style={{ ...th.card, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', padding: '16px 18px', borderBottom: `1px solid ${th.divider}` }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: th.text1 }}>Recent Uploads</div>
              ...
              {recent.length === 0 ? (
                <div style={{ padding: 36, textAlign: 'center', color: th.text3, fontSize: 13 }}>No uploads yet</div>
              ) : (
                    ...
                    <thead>
                      <tr>
                        {['Name', 'Date', 'Location', 'Category'].map(h => (
                          <th key={h} style={{ textAlign: 'left', padding: '10px 18px', fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: th.text3 }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {recent.map(p => {
                        ...
                        return (
                          <tr key={p.id} onClick={() => navigate(`/profiles/${p.profile_id}`)}
                            style={{ cursor: 'pointer', borderTop: `1px solid ${th.divider}` }}
                            ...
                            <td style={{ padding: '11px 18px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                ...
                                <span style={{ fontWeight: 700, fontSize: 13.5, color: th.text1 }}>{p.profile_name || 'Unknown'}</span>
                              </div>
                            </td>
                            <td style={{ padding: '11px 18px', fontSize: 12.5, color: th.text2, whiteSpace: 'nowrap' }}>{fmtDate(p.timestamp)}</td>
                            <td style={{ padding: '11px 18px', fontSize: 13, color: th.text1, maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.address || (p.zip_code ? `ZIP ${p.zip_code}` : '—')}</td>
                            <td style={{ padding: '11px 18px' }}>
