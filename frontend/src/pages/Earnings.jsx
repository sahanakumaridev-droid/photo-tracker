...
export default function Earnings() {
  ...
  const th = {
    ...
    segBg:       isDark ? 'rgba(255,255,255,0.06)' : '#fff',
    segBorder:   isDark ? 'rgba(255,255,255,0.1)'  : '#ececf3',
  }
  ...
  return (
    <div style={{ padding: '20px 28px', height: '100vh', overflowY: 'auto', background: th.pageBg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 800, color: th.text1, letterSpacing: '-0.6px' }}>Earnings</div>
          <div style={{ fontSize: 13.5, color: th.text2, marginTop: 2 }}>Track completed jobs and payouts · Uber-style</div>
        </div>
        {/* Period segmented control */}
        <div style={{ marginLeft: 'auto', display: 'inline-flex', background: th.segBg, border: `1px solid ${th.segBorder}`, borderRadius: 12, padding: 4, gap: 2 }}>
          {PERIODS.map(p => {
      ...
      {loading ? (
        <div style={{ padding: 60, textAlign: 'center', color: th.text3 }}>Loading earnings…</div>
      ) : (
        <>
          {/* Hero + KPIs row */}
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1fr) 1.7fr', gap: 16, marginBottom: 16 }}>
            {/* Dark hero card — intentionally always dark */}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 16 }}>
              {KPIS.map((k, i) => (
                <div key={i} style={{ ...th.card, padding: 18, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  ...
                  <div style={{ fontSize: 28, fontWeight: 800, color: th.text1, letterSpacing: '-1px', marginTop: 8 }}>{k.val}</div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: th.text3, marginTop: 4 }}>{k.sub}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Earnings trend */}
          <div style={{ ...th.card, padding: 18, marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 4 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: th.text1 }}>Earnings Trend</div>
                <div style={{ fontSize: 12, color: th.text3 }}>Daily totals for {periodLabel.toLowerCase()}</div>
              </div>
              <div style={{ marginLeft: 'auto', fontSize: 12, color: th.text2, fontWeight: 700, background: th.surface, padding: '5px 12px', borderRadius: 9 }}>{money(summary?.total_earnings)}</div>
            </div>
            {daily.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: th.text3, fontSize: 13 }}>No completed jobs in this period yet.</div>
            ) : (
                  ...
                  <div key={d.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%' }}>
                    ...
                    <div style={{ flex: 1, width: '100%', maxWidth: 46, margin: '0 auto', display: 'flex', alignItems: 'flex-end' }}>
                      <div title={`${d.date}: ${money(d.amount)}`} style={{
                        ...
                        background: d.amount > 0 ? `linear-gradient(180deg, ${ACCENT}, #a78bfa)` : th.barEmpty,
                        boxShadow: d.amount > 0 ? '0 6px 16px rgba(124,58,237,0.28)' : 'none',
                        transition: 'height .35s ease',
                      }} />
                    </div>
                    <div style={{ fontSize: 11, color: th.text4, fontWeight: 600 }}>{d.date.slice(5)}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ ...th.card, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', padding: '16px 18px', borderBottom: `1px solid ${th.divider}` }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: th.text1 }}>Payouts / Timesheets</div>
              <div style={{ marginLeft: 'auto', fontSize: 13, fontWeight: 800, color: '#10b981' }}>
                {money(payouts?.total_earnings)} · {payouts?.total_jobs ?? 0} jobs
              </div>
            </div>
            {payoutDays.length === 0 ? (
              <div style={{ padding: 36, textAlign: 'center', color: th.text3, fontSize: 13 }}>No closed jobs yet. Mark jobs complete to see payouts.</div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Date', 'Jobs', 'Daily Total'].map(h => (
                      <th key={h} style={{ textAlign: h === 'Daily Total' ? 'right' : 'left', padding: '10px 18px', fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: th.text3 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {payoutDays.map(d => (
                    <tr key={d.date} style={{ borderTop: `1px solid ${th.divider}` }}>
                      <td style={{ padding: '12px 18px', fontSize: 13.5, fontWeight: 600, color: th.text1 }}>{d.date}</td>
                      <td style={{ padding: '12px 18px', fontSize: 13, color: th.text2 }}>{d.jobs}</td>
